...
class SupplyControllerTest {
    ...
    @Mock
    private SupplyControllerAdapter supplyControllerAdapter;
    ...
    void findAll_ShouldReturnOk() throws Exception {
        ...
        when(supplyControllerAdapter.findAll()).thenReturn(List.of(response));

        mockMvc.perform(get("/api/supplies"))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON));

        verify(supplyControllerAdapter, times(1)).findAll();
    }
    ...
    void findByType_ShouldReturnOk() throws Exception {
        var response = createResponse();
        when(supplyControllerAdapter.findByType(SupplyType.SUPPLY)).thenReturn(List.of(response));

        mockMvc.perform(get("/api/supplies/type/{type}", SupplyType.SUPPLY))
                .andExpect(status().isOk());

        verify(supplyControllerAdapter, times(1)).findByType(SupplyType.SUPPLY);
    }
    ...
    void findById_ShouldReturnSupply() throws Exception {
        String id = "123";
        var response = createResponse();
        when(supplyControllerAdapter.findById(id)).thenReturn(response);

        mockMvc.perform(get("/api/supplies/{id}", id))
                .andExpect(status().isOk());

        verify(supplyControllerAdapter, times(1)).findById(id);
    }
    ...
    void create_ShouldReturnCreated() throws Exception {
        ...
        var response = createResponse();
        when(supplyControllerAdapter.create(any(SupplyRequest.class))).thenReturn(response);
        ...
        verify(supplyControllerAdapter, times(1)).create(any(SupplyRequest.class));
    }
    ...
    void update_ShouldReturnOk() throws Exception {
        ...
        var request = createRequest();
        var response = createResponse();
        when(supplyControllerAdapter.update(eq(id), any(SupplyRequest.class))).thenReturn(response);
        ...
        verify(supplyControllerAdapter, times(1)).update(eq(id), any(SupplyRequest.class));
    }
    ...
    void delete_ShouldReturnNoContent() throws Exception {
        String id = "123";
        doNothing().when(supplyControllerAdapter).delete(id);

        mockMvc.perform(delete("/api/supplies/{id}", id))
                .andExpect(status().isNoContent());

        verify(supplyControllerAdapter, times(1)).delete(id);
    }
