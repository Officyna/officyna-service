...
class VehicleControllerTest {
    ...
    @Mock
    private VehicleControllerAdapter vehicleControllerAdapter;
    ...
    void findAll_ShouldReturnOk() throws Exception {
        ...
        when(vehicleControllerAdapter.findAll()).thenReturn(List.of(response));

        mockMvc.perform(get("/api/vehicles"))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON));

        verify(vehicleControllerAdapter, times(1)).findAll();
    }
    ...
    void findById_ShouldReturnVehicle() throws Exception {
        String id = "123";
        var response = createResponse();
        when(vehicleControllerAdapter.findById(id)).thenReturn(response);

        mockMvc.perform(get("/api/vehicles/{id}", id))
                .andExpect(status().isOk());

        verify(vehicleControllerAdapter, times(1)).findById(id);
    }
    ...
    void findByCustomer_ShouldReturnVehicles() throws Exception {
        ...
        var response = createResponse();
        when(vehicleControllerAdapter.findByCustomer(customerId)).thenReturn(List.of(response));

        mockMvc.perform(get("/api/vehicles/customer/{customerId}", customerId))
                .andExpect(status().isOk());

        verify(vehicleControllerAdapter, times(1)).findByCustomer(customerId);
    }
    ...
    void create_ShouldReturnCreated() throws Exception {
        ...
        var response = createResponse();
        when(vehicleControllerAdapter.create(any(VehicleRequest.class))).thenReturn(response);
        ...
        verify(vehicleControllerAdapter, times(1)).create(any(VehicleRequest.class));
    }
    ...
    void update_ShouldReturnOk() throws Exception {
        ...
        var request = createRequest();
        var response = createResponse();
        when(vehicleControllerAdapter.update(eq(id), any(VehicleRequest.class))).thenReturn(response);
        ...
        verify(vehicleControllerAdapter, times(1)).update(eq(id), any(VehicleRequest.class));
    }
    ...
    void delete_ShouldReturnNoContent() throws Exception {
        String id = "123";
        doNothing().when(vehicleControllerAdapter).delete(id);

        mockMvc.perform(delete("/api/vehicles/{id}", id))
                .andExpect(status().isNoContent());

        verify(vehicleControllerAdapter, times(1)).delete(id);
    }
